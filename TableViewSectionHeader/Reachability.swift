...
func callback(reachability:SCNetworkReachability, flags: SCNetworkReachabilityFlags, info: UnsafeMutablePointer<Void>) {
    let reach = Unmanaged<Reachability>.fromOpaque(COpaquePointer(info)).takeUnretainedValue()
...
public class Reachability: NSObject {
    ...
    public func startNotifier() throws {
        ...
        guard let reachRef = reachabilityRef else {
            stopNotifier()
            throw ReachabilityError.UnableToSetCallback
        }
        
        if !SCNetworkReachabilitySetCallback(reachRef, callback, &context) {
            stopNotifier()
            throw ReachabilityError.UnableToSetCallback
        }
        
        if !SCNetworkReachabilitySetDispatchQueue(reachRef, reachabilitySerialQueue) {
            stopNotifier()
    ...
    private let reachabilitySerialQueue = dispatch_queue_create("com.ibm.reachability", DISPATCH_QUEUE_SERIAL)
